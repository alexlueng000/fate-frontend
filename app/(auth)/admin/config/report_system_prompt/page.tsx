...
const CONFIG_KEY = 'report_system_prompt';
...
async function getRevisionDetail(key: string, version: number): Promise<RevisionDetail> {
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => '');
    throw new Error(`加载失败（HTTP ${resp.status}）：${text}`);
  }
  return resp.json();
}
...
async function saveConfig(key: string, value_json: any, comment?: string): Promise<SaveResp> {
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => '');
    throw new Error(`保存失败（HTTP ${resp.status}）：${text}`);
  }
  return resp.json();
}
...
async function rollbackConfig(key: string, version: number, comment?: string): Promise<SaveResp> {
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => '');
    throw new Error(`回滚失败（HTTP ${resp.status}）：${text}`);
  }
  return resp.json();
}

export default function ReportSystemPromptPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) { router.push(`/login?redirect=/admin/config/${CONFIG_KEY}`); return; }
      const u = JSON.parse(raw) as Me;
      if (!u?.is_admin) { router.push(`/login?redirect=/admin/config/${CONFIG_KEY}`); return; }
      setMe(u);
    } catch {
      router.push(`/login?redirect=/admin/config/${CONFIG_KEY}`);
      return;
    }
    ...
    (async () => {
      ...
      try {
        ...
        // Key might not exist yet — start empty
        setContent('');
        setNotes('');
        setMsg('尚未配置报告专用提示词，可在此创建首个版本。');
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function onSave() {
    setBusy(true); setMsg('');
    try {
      const res = await saveConfig(CONFIG_KEY, { content, notes }, `update ${CONFIG_KEY}`);
      setVersion(res.version);
      const fresh = await getConfig(CONFIG_KEY);
  ...
  async function onRollback(v: number) {
    ...
    setBusy(true); setMsg('');
    try {
      const res = await rollbackConfig(CONFIG_KEY, v, `rollback to v${v}`);
      const fresh = await getConfig(CONFIG_KEY);
  ...
  function closeDetail() { setDetailOpen(false); setDetailRev(null); }
    ...
    <main className="min-h-screen bg-[#fff7e8] text-neutral-800">
      <header className="sticky top-0 z-10 backdrop-blur bg-[#fff7e8]/80 border-b border-[#f0d9a6]">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          <h1 className="text-xl md:text-2xl font-bold text-[#a83232]">报告页提示词</h1>
      ...
      <section className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-[#f0d9a6] bg-white/90 p-3 flex flex-col">
          <div className="flex items-center justify-between mb-2 gap-2">
            <div className="text-[#4a2c2a] text-sm">编辑 · 字数 {charCount}</div>
            <input
              value={notes}
        ...
        <div className="rounded-2xl border border-[#f0d9a6] bg-white/90 p-3 flex flex-col">
          <div className="mb-2 text-[#4a2c2a] text-sm">预览</div>
        ...
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-4xl max-h-[85vh] rounded-2xl bg-white shadow-xl border border-[#f0d9a6] flex flex-col">
            ...
            <div className="px-5 py-3 border-t border-[#f0d9a6] flex justify-between">
              ...
              {detailRev && (
                <button
                  onClick={() => { closeDetail(); onRollback(detailRev.version); }}
