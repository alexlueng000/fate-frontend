...
export function normalizeMarkdown(md: string): string {
  ...
  const fixed = parts.map((seg) => {
    ...
    s = s.replace(/^(#{5,6})[ \t]*/gm, "#### ");                // 5-6级 -> 4级
    s = s.replace(/^(#{1,2})[ \t]*/gm, "### ");                 // 1-2级 -> 3级（如不想统一可删）
    ...
    // 3.1 标准化各种横线字符：全角减号/短横/长横 -> 普通减号（仅用于结构化规则匹配；不会改正文里的单个破折号）
    // 行首用作列表标记：把 "— / – / －" 视为 "- "
    s = s.replace(/^[ \t]*[—–－][ \t]*/gm, "- ");

    // 3.2 句中起列表：……(空格)(-|—|–|－) 文本  -> 断到新行
    s = s.replace(new RegExp(`([^\\n])\\s+([\\-—–－])\\s+`, "g"), "$1\n\n- ");

    // 3.3 列表标记后的连字符噪声："- --- 文本" / "- —— 文本" / "- －－ 文本" -> "- 文本"
    s = s.replace(/^-\s+[—–－-]{2,}\s*/gm, "- ");

    // 3.4 标准化 "-一、" -> "- 一、"（中文编号紧贴的情况）
    s = s.replace(/^[ \t]*-([^\s])/gm, "- $1");

    // 3.5 标题后紧贴列表 -> 补空行
    s = s.replace(/(#{3,4}\s[^\n]+)\n(- |\d+\.\s)/g, "$1\n\n$2");

    // 3.6 禁用水平分割线（整行由 3+ 个横线组成，不管是 - / — / – / －）
    s = s.replace(/^\s*[—–－-]{3,}\s*$/gm, "");

    // 3.7 句首/行首的“——（破折号引导语）”如果你不想显示，直接去掉
    s = s.replace(/^[ \t]*[—–－]{2,}[ \t]*/gm, ""); // 可按需注释

    // 4) 免责声明独立一段
    s = s.replace(/(以上内容由传统文化AI生成[^\n]*)/g, "\n\n$1\n\n");
